...
namespace pcpp
{
	...
	public:
		template <typename T>
		using EnableIfUnsignedIntegral = std::enable_if_t<std::is_integral<T>::value && std::is_unsigned<T>::value, int>;
	...
	public:
		...
		explicit Asn1UtcTimeRecord(const std::tm& value, bool withSeconds = true) : m_Value(value), m_WithSeconds(withSeconds)
		{}

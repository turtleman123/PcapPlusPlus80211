...
PTF_TEST_CASE(Asn1DecodingTest)
{
	...
	// Integer 1 byte
	{
		...
		PTF_ASSERT_EQUAL(record->castAs<pcpp::Asn1IntegerRecord>()->getIntValue<uint8_t>(), static_cast<uint8_t>(6));
...
PTF_TEST_CASE(Asn1EncodingTest)
{
	...
	// Big Integer
	{
		std::string recordValue = "21c28a1bff4aa8400226fc73409b54bbc1f06c5f";
		...
		PTF_ASSERT_EQUAL(record.getTotalLength(), 22);
		PTF_ASSERT_EQUAL(record.getValueLength(), 20);
		PTF_ASSERT_EQUAL(record.getValueAsString(), recordValue);
